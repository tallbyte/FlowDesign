...
import com.tallbyte.flowdesign.data.flow.FlowComment;
 ...
 */
public class FlowCommentElementNode extends ElementNode<DiagramImage> {

    public static final String PSEUDO_CLASS_REFERENCED = "referenced";

    protected ObjectProperty<?> reference;

    public FlowCommentElementNode(FlowComment element, DiagramImage content) {
        super(element, content, Pos.CENTER);


        try {
            reference = JavaBeanObjectPropertyBuilder.create().bean(element).name(FlowComment.PROPERTY_REFERENCE).build();
